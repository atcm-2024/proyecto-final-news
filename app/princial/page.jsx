...
export default function page({data}) {
  ...
  console.log(articles)
    ...
    return (
      <div className="w-full h-[71%] bg-slate-800 text-white flex overflow-y-auto justify-center items-center">
        <div className="h-full w-[95%] grid grid-cols-1 mt-12 md:grid-cols-3 gap-2 justify-center items-center">
          {articles?.length > 0 ? (
            articles.map((item, index) => {
              // Determine the default image based on category
              let urlAlterna = "/gestion-de-negocios.jpg"; // Default image
                  ...
                  >
                    ...
                    <img
                     ...
                     <div className="h-1/2  w-[95%] mt-5 flex flex-col columns-1">
                        ...
                        <label className="mt-2 mb-7 ">
                          Fuente: <a href={item.url} target='_blank'>{item?.source?.name || item?.name}</a>
                        </label>
                      </div>  
                </div>
                );
